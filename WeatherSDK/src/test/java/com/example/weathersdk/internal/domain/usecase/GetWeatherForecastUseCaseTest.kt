...
class GetWeatherForecastUseCaseTest {
    ...
    @Test
    fun `invoke should return success when both forecast and hourlyForecast are successful`() =
        runBlocking {
            // Arrange
            val city = "TestCity"
            val forecast =
                Forecast(temperature = 25.0, timestamp = "10:00 AM", description = "Sunny")
    ...
    fun `invoke should return failure when hourlyForecast fails`() = runBlocking {
        ...
        coEvery { repository.getForecast(city) } returns Success(forecast)
        coEvery { repository.getHourlyForecast(city) } returns Failure("")
